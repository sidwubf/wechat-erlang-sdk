...
get(AccessToken) ->
    URL = list_to_binary(?API_URL_PREFIX ++ "/menu/get?access_token="++AccessToken),
    wechat_util:http_get(URL).

delete(AccessToken) ->
    URL = list_to_binary(?API_URL_PREFIX ++ "/menu/delete?access_token="++AccessToken),
    wechat_util:http_get(URL).
...
get_info(AccessToken) ->
    URL = list_to_binary(?API_URL_PREFIX ++ "/get_current_selfmenu_info?access_token="++AccessToken),
    wechat_util:http_get(URL).
