...
get_access_token(AppId, AppSecret) ->
    URL = list_to_binary(?API_URL_PREFIX ++ "/cgi-bin/token?grant_type=client_credential&appid="++AppId++"&secret="++AppSecret),
    wechat_util:http_get(URL).

%{"ip_list": ["127.0.0.1", "127.0.0.2", "101.226.103.0/25"]}
get_callback_ip(AccessToken) ->
    URL = list_to_binary(?API_URL_PREFIX ++ "/cgi-bin/getcallbackip?access_token="++AccessToken),
    wechat_util:http_get(URL).
